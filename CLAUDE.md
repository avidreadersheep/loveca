...
| Phase 3b PC 盤面 UI | **実装中**（設計は `docs/盤面設計メモ.md` / 決定 D73〜D79）。★**M-B1 = 完了 / 決定 D80・D81**（R7 + `GameStore` + 6.2.1 の開始手順 + 静的な盤面描画）。★**M-B2 = 完了 / 決定 D82〜D85**（物理操作のドラッグ + `DropEdge` + 反転 / 向き / 重ね置きのメニュー。★**U5 / U16 / U17 を実測で解消**）。★**M-B3 = 完了 / 決定 D86・D87**（進行 `AdvanceStep` + 8.3.6・8.4.12 の分岐 + 集計 + 警告 + 引く / シャッフル / 上から見る。★**のべ 73 ステップを 1 つ残らず通した**。★D83 を測り直し、★**縦の下限を初めて測って新設 U19**）。**次は M-B4（★モード / 決定 D88。旧 M-B4 の巻き戻しは M-B5 へ繰り下げ）** |

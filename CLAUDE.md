...
| `loveca-ui`（Flutter） | 431（★skip 0） | `flutter test` |

（4 パッケージとも 2026-08-24 時点 / Phase 3b の M-B1 完了時）
...
| Phase 3b PC 盤面 UI | **実装中**（設計は `docs/盤面設計メモ.md` / 決定 D73〜D79）。★**M-B1 = 完了 / 決定 D80・D81**（R7 + `GameStore` + 6.2.1 の開始手順 + 静的な盤面描画。★実データで起動確認済み）。**次は M-B2** |
...
**次の一手**: ★**M-B1 は完了。次は M-B2**（物理操作 = ドラッグ / 反転 / 向き / 重ね置き +
`DropEdge` + 画像の段の実測）。設計は `docs/盤面設計メモ.md` §11。

★**M-B2 で解く未決が 2 つある**（どちらも実測で決める / U8 と同じ手順）。
- **U5**: スロットの物理幅が thumb の原寸 200px を超えるか（★**決定 D76 で種別では割れない**）
- **U16**: 盤面の最小ウィンドウ幅。★**暫定値 `kBoardMinWidth = 1100` は既に置いてあり、
  その幅で溢れないことはテストで固定済み**（`test/board/board_layout_test.dart`）。
  残るのは**物理px と可読性の実測**

★**M-B1 で片づいたもの**（読み直さなくてよい）:
`DrawEnergy`（D73）/ ゲーム開始手順 6.2.1（D79 / D80）/ **D-15 の (a)(b)(c)(d) 全件** /
**D-2**（`loveca-core` にリントを接続。★除外設定は 1 つも要らなかった）。

★★**M-B1 で決めたことが 2 つある**（`docs/決定事項一覧.md` D80 / D81）。**やり直さないこと。**
...
| **Phase 3b（M-B2）** | **U17** | ★**相手の手札を常時出したままにするか。**`docs/盤面設計メモ.md` §4-3 は「相手の手札は枚数だけ / 中身を開く口を別に置く」と書いていたが、★**M-B1 はそこから外れて帯として常時出している**（口の仕組みがまだ無く、隠すと D77 の「一人回しでは両方見える」に到達できないため）。★**どちらでもよいのではなく、決めていないだけである** | `docs/盤面設計メモ.md` §4-3 / §13 |
| **Phase 3b（M-B2）** | **U16** | ★**盤面の最小ウィンドウ幅。**置き場 11 + 共有 1 が同時に見える必要があり、`PaneScaffold` の 840（D61）とは別の値になる。★**M-B1 で暫定値 `kBoardMinWidth = 1100` を置き、その幅で溢れないことはテストで固定済み。**残るのは**物理px と可読性の実測** | `docs/盤面設計メモ.md` §13 / `loveca-ui/lib/src/ui/board/board_slot.dart` |

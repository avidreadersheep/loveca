...
**次の一手**: ★**M-B3 は完了。次は M-B4**（巻き戻し = `undo` / `undoStep` を別ボタン +
合成コマンドを 1 record + §8-3 の乱数の注記）。設計は `docs/盤面設計メモ.md` §8 / §11。

★★**M-B4 で `loveca-ui/test/state/board_session_test.dart` を置くこと。**★★
...
`BoardNotice.MulliganNotImplemented` が盤面に常設してある。**M-B5 で実装したらこの枝ごと消すこと。**

...
| `loveca-ui`（Flutter） | 358（★skip 0） | `flutter test` |
...
2. §10 の **U11**: 一覧はライブのセルを `cover` で切っている。比を変えると
   **セル幅 120 物理px という D42 の測定条件が動く**
3. §7 の 5 項目（モバイル）。★**未了である限り Release 1 を「完了」と呼べない**
...
**新しい決定は D72 以降を使う（D36〜D71 は使用済み）。** 未記録番号を再利用しないこと。
...
| **一覧の見た目を次に触るとき** | **U11** | ★**一覧はライブのセルを `cover` で切っている。**thumb を全数計測したところ メンバー / エネルギーは 200×279 の縦長だが**ライブは 200×143 の横長**で、全セル 200:279 固定なので左右が切れる。比を変えると**セル幅 120 物理px という D42 の測定条件が動く**（`ResizeImage` の効果もキャッシュの見積りも測り直し） | `docs/UI設計メモ.md` §5-1 / §9-9 の U11 |

...
| **判断待ち**（本タスクで提案済み） | **D-10** | ★**`.gitignore` の未アンカー規則が本実装を隠しうる。**`build/`（root）と `data/`（loveca-data）が残っている。★**`git status` / テスト / analyze / 実起動のすべてが正常を報告するのに、クローンするとビルドできない** | `ルール整合性チェック_v1.06.md` D-10 |

...
**次の一手**: ★**Phase 2 後半は完了。Release 1 の機能が揃った。**
次は **Phase 3b（PC 盤面 UI）**。★着手時に **B-4**（ポジション / フォーメーションチェンジ）・
**B-2**（エネルギーデッキの順序）・**D-2**（`loveca_core` にリントが 1 つも効いていない）を読むこと。
...
**新しい決定は D73 以降を使う（D36〜D72 は使用済み）。** 未記録番号を再利用しないこと。

...
| `loveca-ui`（Flutter） | 198（★skip 0） | `flutter test` |
...
| Phase 2 PC ローカル DB・カードリスト・デッキ構築 | **着手中**（第一段階: drift スキーマ + マスタ取り込み層 = 完了。UI 着手前の技術検証 = 完了 / `docs/UI技術検証メモ.md`。検索の改善と移行機構 = 完了 / D49・D50。UI 本実装の設計 = 完了 / `docs/UI設計メモ.md`・D52〜D61。**M1 土台 + R1 + R4 = 完了。M2 R2/R3 最小版 + デッキの書き込み = 完了 / D62・`docs/UI設計メモ.md` §9-4。M3 検索 = 完了 / D63・D64・`docs/UI設計メモ.md` §9-5。M4 R3 デッキ編集 + P1 = 完了 / D65・`docs/UI設計メモ.md` §9-6〜§9-8（★U8 を実測で解消・D-11 / D-12 を記録）**） |
...
**次の一手**: Phase 2 後半の **M5（R5 カード詳細。PC はペイン / モバイルはルート）**。
`loveca-ui/lib/` に書く。
★**M5 で確認するのは「ペイン抽象が 2 通りの器で成立すること」**（`docs/UI設計メモ.md` §2-4）。
★**M4 で骨格は通った**（§9-6）。R3 は一覧ペインを R4 と共有しているので、
**R5 を足すときも同じ Widget を器だけ替えて置く**（§2-2「ルートを増やして分岐させない」）。
★M5 の着手時に読むこと: `docs/UI設計メモ.md` §2-7(1)
（**`Faq.cardNumbers` の中身は cardNumber ではなく printingId**。
フィールド名を信じて結合すると**例外も出ずに 0 件**になる）。
設計は `docs/UI設計メモ.md`（決定 D52〜D65）、描画の実測は `docs/UI技術検証メモ.md`（決定 D42〜D48）。
そこから外れる場合は理由を示すこと。
`loveca-ui/spike/` は検証用であり、**本実装から参照しない。**

決定事項の参照先は `docs/決定事項一覧.md`（`決定 DNN` / `決定 D-X` の実体）。
**新しい決定は D66 以降を使う（D36〜D65 は使用済み）。** 未記録番号を再利用しないこと。

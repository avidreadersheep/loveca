...
| `loveca-ui`（Flutter） | 515（★skip 0） | `flutter test` |

（4 パッケージとも 2026-08-25 時点 / Phase 3b の M-B2 完了時）
...
| Phase 3b PC 盤面 UI | **実装中**（設計は `docs/盤面設計メモ.md` / 決定 D73〜D79）。★**M-B1 = 完了 / 決定 D80・D81**（R7 + `GameStore` + 6.2.1 の開始手順 + 静的な盤面描画）。★**M-B2 = 完了 / 決定 D82〜D85**（物理操作のドラッグ + `DropEdge` + 反転 / 向き / 重ね置きのメニュー。★**U5 / U16 / U17 を実測で解消**。★実データで起動し、ドラッグ・重ね置き・帯の切替を目視確認済み）。**次は M-B3** |
...
**次の一手**: ★**M-B2 は完了。次は M-B3**（進行 = `AdvanceStep` / 8.3.6・8.4.12 の分岐 +
集計表示 + 警告）。設計は `docs/盤面設計メモ.md` §11。

★★**M-B3 に「引く / シャッフル / 上から見る」も移してある。**★★
CLAUDE.md §1（D-A）が許す物理操作のうち、**引く（5.6）/ シャッフル（5.5）/
上から見る（5.7.1 / 5.7.2 / 10.2.2.2）**は M-B2 の列挙に無かったので入れていない。
★**忘れたのではない**（盤面設計メモ §11 の M-B3 行に明記した）。

★★**エネルギーデッキ置き場（4.9）に「上から見る」の口を作ってはならない。**★★
5.7.1 / 5.7.2 / 10.2.2.2 はいずれも**メインデッキ置き場（4.8）**の規定で、
**4.9 を対象にした条は存在しない**（決定 D73 / 盤面設計メモ §2-5）。
★**対称性から付けたくなるので、実装する段（M-B3）に書いてある。**

★**M-B1 / M-B2 で片づいたもの**（読み直さなくてよい）:
`DrawEnergy`（D73）/ ゲーム開始手順 6.2.1（D79 / D80）/ **D-15 の (a)(b)(c)(d) 全件** /
**D-2**（`loveca-core` にリント接続）/ **U5**（D82）/ **U16**（D83）/ **U17**（D84）。

★★**M-B1 / M-B2 で決めたことは `docs/決定事項一覧.md` D80〜D85 にある。やり直さないこと。**★★
...
**新しい決定は D82 以降を使う（D36〜D81 は使用済み）。** 未記録番号を再利用しないこと。
...
| 済 | **U16** | ~~盤面の最小ウィンドウ幅~~ → ★**決定 D83 で解消**（2026-08-25）。**1100 据え置き。**(a) 外部標準は**無い**（D75 がペイン縮退を禁じている）/ (b-1) 溢れの下限 506・530 / (b-2) ★**条文由来の下限 696**（6.2.1.5 の初期手札 6 枚が同時に見える）/ (b-3) ★**札の大きさは窓幅で変わらない**ので可読性は `kBoardSlotWidth` の話。★**測定手法に欠陥があり U8 の記録値も 151 → 198 に訂正** | **解消**（`docs/決定事項一覧.md` D83） |

...
| `loveca-ui`（Flutter） | 240（★skip 0） | `flutter test` |
...
**次の一手**: Phase 2 後半の **M6（R6 設定・診断 + P3 メタ編集 + 共有形式の入出力）**。
`loveca-ui/lib/` に書く。
★**M6 で塞がるのは D39 / D35 の「出口」**（`docs/UI設計メモ.md` §2-4）。
取り込み失敗（`import_issues`）を出す先が無いままなので、**記録しているのに誰も見ない**状態が続いている。

★**M6 の着手時に読むこと（3 つとも罠がある）**
1. §2-5 **共有形式の入力**: cardNumber → printingId の逆写像が**一意でない**
   （★通常刷りが複数ありうる / CLAUDE.md §5-(4)）。既定の刷りの選び方は**未決 U7**。
   ★**未知 cardNumber を黙って捨てない**（§2-5(b)）
2. §10 の **U9**: 論理削除したデッキを**戻す口が無い**。R6 を作るならここで判断する
3. §2-7(1) **FAQ を出すなら**: `Faq.cardNumbers` の中身は
   **cardNumber ではなく printingId**。名前を信じて結合すると**例外も出ずに 0 件**になる
   （★M5 では触っていない）

★**M4 / M5 で骨格と器はできている。** R3 / R4 / R5 は同じ一覧ペインと
同じ詳細ペインを器だけ替えて置いているので、**M6 でもルートを増やして分岐させない**（§2-2）。
設計は `docs/UI設計メモ.md`（決定 D52〜D66）、描画の実測は `docs/UI技術検証メモ.md`（決定 D42〜D48）。
そこから外れる場合は理由を示すこと。
`loveca-ui/spike/` は検証用であり、**本実装から参照しない。**

決定事項の参照先は `docs/決定事項一覧.md`（`決定 DNN` / `決定 D-X` の実体）。
**新しい決定は D67 以降を使う（D36〜D66 は使用済み）。** 未記録番号を再利用しないこと。

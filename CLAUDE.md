...
★**コード・テストの文言は M-B4 の実装で直す**（それまでは残る）。

**次の一手**: ★**M-B3 は完了。次は M-B4（モード）**——ソロ / ローカル対戦の実装 +
「一人回し」の語の置き換え + ★**dist が解決できていないことの可視化**（決定 D89 / 別コミット）。
設計は `docs/盤面設計メモ.md` **§14** と `docs/UI設計メモ.md` **§11**（決定 **D88 / D89**）。
...
★**コード・テストの中の `M-B4` / `M-B5`（★実測 **18 行**）は M-B4 の実装で直す**（`docs/盤面設計メモ.md` §11）。
...
★**M-B1 / M-B2 / M-B3 で片づいたもの**（読み直さなくてよい）:
...
★★**M-B1 / M-B2 / M-B3 で決めたことは `docs/決定事項一覧.md` D80〜D87 にある。やり直さないこと。**★★
...
9. ★**dist が解決できていないことを 3 層で見せる**（D89 / `docs/UI設計メモ.md` §11）。★**測定は M-B4 の実装時**
...
| **盤面の縦の見た目を次に触るとき** | **U19** | ★**盤面の最小の高さ。**進行バー・帯・集計は**縦にスクロールしない**ので、窓を縦に狭めると固定の段が溢れる。★**2026-08-25 に初めて測った = 496 論理px**（内訳: `AppBar` 56 / 進行バー 48 / 常設の帯 160 / 集計 220）。★**値を推測で埋めない。**幅（D75）は「盤面ごと横スクロール」で解いたが、**縦を同じ形にすると `BoardLayout` の中のスクロールと入れ子になる** | `docs/盤面設計メモ.md` §13 |
| ★**M-B4 の実装時** | **U20** | ★**ソロの最小幅・最小高さ。**ソロは置き場が減るので D83 の下限も U19 も下がる方向だが**測っていない**。`kBoardMinWidth = 1100` はソロでは過剰かもしれない。★**値を推測で埋めない**（D83 と同じ手順で測る）。★**(b-3) 696 は条文由来（6.2.1.5）なのでソロでも効く** | `docs/盤面設計メモ.md` §13 |

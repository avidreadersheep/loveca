...
HEART_NAME = {
    ...
    "無": GRAY,
    "ALL": ALL,
    # ★パラレル刷りだけが使う ALL の別表記★
    #  実データ 5 刷り (PL!HS-bp2-022-L＋ など) が attack='[全ブレード]' で、
    #  同一 cardNumber の通常刷り -L はいずれも attack='ALL1' かつスコアも同値。
    #  総合ルール上「同一カードナンバー = 同一のカード」なので ALL 1 個と解釈する。
    #  ※条文の定義ではなく実データからの推論。V15 が刷り間の一致を検証している。
    "全ブレード": ALL,
}
...
KEYWORD_TOKENS = {
    ...
    "【右サイド】": "RIGHT_SIDE",
    # ★括弧の表記ゆれ★
    #  角括弧は extract_keywords 側の NFKC 正規化で吸収する
    #  (［ターン1回］ 全角 -> [ターン1回] 半角)。
    #  墨括弧 【】 は NFKC で変化しないため、ここに個別に持つ必要がある。
    #  ターン2回は ver 1.05 追加で該当が 6 件しかなく、
    #  全角表記は現データに 0 件だが、出た瞬間に取り逃さないよう先に入れておく。
    "[ターン1回]": "TURN_1",
    "【ターン1回】": "TURN_1",
    "[ターン2回]": "TURN_2",
    "【ターン2回】": "TURN_2",

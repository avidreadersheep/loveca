...
class Deck {
  ...
  ///
  /// ## ★★ 2026-09-02 追記: ★★D-20 とは★格が違う（★運転指示【0】(5)）★★
  ///
  /// ★★**上の 6 行は 1 文字も書き換えない**★★（**D-35** —— ★★走査の結果は真である★★）。
  /// ★**足りないのは★★「同じ形」と「同じ格」を分けること★★である**（**§7-7**）。
  ///
  /// ★★ D-20 が疑っているのは「呼び出し側が 0 人であること」ではない ★★
  /// ★**あの節が書いているのは★★`Zone.visibility` が★★総合ルール 4.1.2.1 を 1 度も実装しないまま来た★★**である ——
  ///   ★**「属性を定義した」ことで★★実装されたと信じられていた★★**（★★用途が★誰にも決まっていなかった★★）。
  /// → ★★**D-20 の型は「★使われていないのに★★正しいと信じている★★」である。**★★
  ///
  /// | | ★**D-20**（`Zone.visibility`） | ★★**この口**★★ |
  /// |---|---|---|
  /// | ★呼び出し側 | ★**0 人** | ★**0 人**（★同じ） |
  /// | ★★**用途**★★ | ★★**決まっていなかった**★★（★誰も要求しなかった） | ★★**決まっている**★★（★上の表 —— ★手元のファイルを読む） |
  /// | ★★**何を信じていたか**★★ | ★★**条文が実装されていること**★★（★★偽だった★★） | ★★**信じているものが無い**★★（★★用途と★使わない用途を★両方書いてある★★） |
  /// | ★検知手段 | ★**無い**（★`dart analyze` は公開 API の未使用を報告しない / ★実測） | ★**走査が 2 か所に在る**（`deck_wire_test.dart` / `deck_sync_test.dart`） |
  ///
  /// ★★ 消す条件を★先に書く（★★「そのうち考える」にしない★★）★★
  /// ★★**消すのは、★用途が★★実際に現れたときに★形が合わないと判ったとき★★である。**★★
  /// ★**「呼び出し側が 0 人であること」は★★消す理由にならない★★** ——
  ///   ★**0 人であることは★★今日この口を通る経路が無い★★という事実であって、
  ///     ★★この口が誤っているという判定ではない★★。**
  /// ★★**「いつか使うかもしれないから残す」とも書かない**★★ —— ★**上の表の 用途 が★根拠である。**

...
enum StepId {
  ...
  ///
  /// ★7.7.2「手番プレイヤーにプレイタイミング (9.5.2) が与えられます」/
  ///   7.7.2.1「起動能力を **1つ選び**、それをプレイする」/
  ///   7.7.2.2「自分の手札のメンバーカードを **1枚選び**、それをプレイする」
  ///   = プレイヤーの選択。★フェイズ構造の中で唯一のプレイタイミングである。
  ///   ★通過すると盤面が育たない。
  s7_7_2('7.7.2', checkTiming: true, requiresPlayerAction: true),
  ...
  ///
  /// ★8.2.2「自身の手札のカードを **3枚まで選んで**裏向きに自身のライブカード置き場に
  ///   置き、置いた枚数と同じ枚数カードを引きます」= プレイヤーの選択と盤面操作。
  ///   ★通過すると 8.3.4 が空のライブカード置き場を読み、8.3.6 が早期終了する。
  s8_2_2('8.2.2', requiresPlayerAction: true),
  ...
  ///   ★同じ一文が「〜を **3枚まで選んで**…置き」とプレイヤーの選択も求めている
  ///   ([requiresPlayerAction])。★ソロでは [requiresOpponent] により通らないので、
  ///   そもそも停止点にならない。
  s8_2_4('8.2.4', requiresOpponent: true, requiresPlayerAction: true),
  ...
  ///
  /// ★8.3.15.1.1「その際、各アイコンは、**任意の色**のハートアイコン 1つとして扱う
  ///   ことができます」= ALL の色割り当ては**プレイヤーの選択**である (決定 D18)。
  s8_3_15('8.3.15', requiresPlayerAction: true),

  /// いずれかが満たせなかった場合、ライブカード置き場のすべてのライブカードを控え室へ。
  ///
  /// ★8.3.16「前述の手順によりいずれかのライブカードの必要ハートを満たすことが
  ///   できなかった場合、ライブカード置き場のすべてのライブカードを自身の控え室に
  ///   置きます」= 満たせたかの判断 (決定 D18) と**盤面操作**。
  ///   ★通過すると 8.4.2 が消えるはずのスコアを数える。
  s8_3_16('8.3.16', requiresPlayerAction: true),
  ...
  ///
  /// ★8.4.6「合計スコアを比較し、ライブに勝利したプレイヤーを決定します」
  ///   = **勝敗の確定**であり手動である (決定 D10)。
  ///   ★効果でライブカードのスコア値が増減しうるためアプリは計算できない (D18)。
  s8_4_6('8.4.6', requiresOpponent: true, requiresPlayerAction: true),
  ...
  ///
  /// ★8.4.7「ライブに勝利したプレイヤーは、自身のライブカード置き場のカードを
  ///   **1枚選び**、それを自身の成功ライブカード置き場に移動します」
  ///   = プレイヤーの選択と盤面操作 (D10 / D18)。★ここが手動判定の着地点である。
  ///   ★通過すると `liveJudgement` が立たず 8.4.13 が入れ替えを行わない。
  s8_4_7('8.4.7', requiresPlayerAction: true),
  ...
  /// ★★ 条文がそのステップでプレイヤーの選択・判断・盤面操作を求めているか (決定 D92) ★★
  ///
  ///   自動進行 ([stopsAutoAdvance]) は**このステップの手前で止まる**。
  ///   ★**7 件だけ** —— 7.7.2 / 8.2.2 / 8.2.4 / 8.3.15 / 8.3.16 / 8.4.6 / 8.4.7
  ///   (各宣言の直上に該当句を引いてある)。
  ///
  /// ★★ 「アプリが代行しない」ことまでが条件である ★★
  ///   根拠は D-A (効果の自動処理を実装しない) / D10 (勝敗確定は手動) /
  ///   D18 (ライブ成功判定は手動)。**条文に「選ぶ」と書いてあっても、
  ///   アプリが実行するなら止まらない** (8.3.12 のドローがそれである)。
  ///
  /// ★★ 8.4.12 には立てない ★★
  ///   すでに [decision] が [StepDecision.playerDeclared] で答えている。
  ///   同じ事実を 2 箇所に書かない (`ルール整合性チェック_v1.06.md` D-15)。
  ///
  /// ★★ 「プレイヤーが出てくる語を含む」ではない ★★
  ///   止まらないものを対で挙げる (`docs/盤面設計メモ.md` §15-3)。
  ///   - **誘発ステップ** (7.4.2 / 7.5.1 / 7.6.1 / 7.7.1 / 8.2.1 / 8.3.3 / 8.3.8 /
  ///     8.4.1 / 8.4.4 / 8.4.10) …条文は「誘発条件が**発生します**」= **事象**であって
  ///     プレイヤーの行動ではない。プレイと解決はチェックタイミング (9.5.3.2) で起きる
  ///   - **8.3.10 / 8.3.12 / 8.3.14 / 8.4.2** の集計 …すでに画面の帯に出ている (D18)。
  ///     ★8.3.12 の**ドローはアプリが実行する** (`step_engine.dart`)
  ///   - **8.4.3** …「合計スコアを比較する場合、それは以下の手順で実行します」=
  ///     **比較の規約定義**であって行動ではない
  ///   - ★**8.3.6** …**分岐**であって停止点ではない ([StepDecision.automatic])
  ///   - ★**8.4.13 / 8.4.14** …**アプリが実行する** (`_swapFirstPlayer` / `_endTurn`)。
  ///     8.4.14 の直後で止まるのは**動的停止 (実装判断)** であって条文由来ではない
  final bool requiresPlayerAction;

  /// 自動進行がこのステップの**手前で**止まるか (決定 D92)。
  ///

...
/// **正は実機の値**で、`docs/UI設計メモ.md` §9-7 に置く（★**§9-6 ではない**）。
...
///
/// ★★★ 2026-08-27（束 B / 決定 D99）: 198 → **288** に動いた ★★★
/// ★**これは測定手法の訂正ではなく、実装が変わったのである。**上の訂正と混ぜないこと。
/// `_Header` に「規則順に戻す」の `TextButton.icon` を 1 つ足したぶんで、
/// ★**外して測り直すと 198 に戻ることを実測した**（2026-08-27）。
///
/// ★★★ ここで測る 288 は「格(1)」である。格(2) にも 288 があるが別の量 ★★★
/// `docs/UI設計メモ.md` §9-7 の格(2) は**実機の実データで刷り番号の行が読める下限**で、
/// 2026-08-24 の記録が偶然おなじ **288** だった。★**同じ数だからと束ねないこと。**
/// ★★2026-08-27 に格(2) を測り直した★★ —— M4 の「288」は**2 点だけ見た値**であって
/// 下限ではなかった。★**値はここに書かない。正は §9-7 である**（同じ数を 2 箇所に書かない）。
///
/// ★★ 採用値 320 との比べ方 ★★
/// ★**このファイルが持つのは「320 で溢れないこと」の回帰ガードだけ**である（下のテスト）。
/// ★★**採用値 320 の「余裕」を、ここで測る値から引いて計算してはならない。**★★
/// 320 の根拠は**格(2)**（実機の可読性）であって格(1) ではない。
/// ★束 B の報告が「余裕が 122 → 32 に減った」と書いたのは**格をまたいだ引き算**で、誤りである
/// （正しい引き算は格(2) から。★`docs/UI設計メモ.md` §9-7）。
///
/// ★★ 決定 D61 の結論は変わらない ★★
/// 採用値 `kDeckPaneMinWidth = 320` はどちらの値も上回っており、
/// 「2 ペインにした瞬間に一覧が 3 列を割らない」という根拠 (b) は成立したまま。
/// **変わったのは記録されていた数値であって判断ではない。**
/// ★2026-08-27 の格(2) の測り直しでも変わらない。
///
...
/// 全区分 + 未知の刷り + 並べ替え済みという**最も混んだ状態**で測る。
///
/// ★★ 2026-08-27: 縮退は 2 本ではなく 1 本になった（決定 D99）★★
/// `DeckOrderNotPersisted`（「開き直すとカード番号順に戻ります」）を撤去したので、
/// 並べ替えても縮退は増えない。残るのは `DeckUnknownPrintings` だけである。
/// ★**下の `moveEntry` は残してある** —— 縮退のためではなく
/// 「手で並べ替えた状態」そのものを作るためで、`_crowdedDeck` の意図は変わらない。

...
void main() {
  ...
  group('★★ 陽性対照 —— ★取り出しが働くこと（**D-10**）★★', () {
    ...
    test('★★ 「この節が正である」の節は★取り出さない（★理由のある除外 / D-30）★★', () {
      expect(questionText.contains('★★この節が正である★★'), isTrue,
          reason: '★除外の相手が実在すること');
      for (final heading in copied.keys) {
        expect(heading.contains('この節が正である'), isFalse);
      }
    });
  });

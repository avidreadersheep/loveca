...
void main() {
  ...
  group('★★ 陽性対照 —— ★取り出しが働くこと（**D-10**）★★', () {
    ...
    test('★★ 「写し」の節の取り出しが働く（★合成の入力で見る / D-27）★★', () {
      // ★★ 2026-09-04: ★実物からこの形が★0 件になった ★★
      //   ★**12 の問いに一度に回答が返り、★★写しを持つ 6 つが★すべて閉じた★★**
      //   （**D150** / `docs/同期設計メモ.md` §9-20）。
      //   ★**残る 3 つ（Q-06 / Q-08 / Q-16）は★★どれも「この節が正である」形である★★。**
      //   → ★**実物だけで測ると★★この取り出しに対が 1 つも付かない★★**（★型は **D-27** の (J)）。
      //   ★★**先例は★すぐ下の「この節が正である」の群**★★（2026-09-01 に★同じ処置をした）。
      final rows = _copiedRows(const <String>[
        '#### ★ 問いの文言（★★写し。★正は §99 の **N-99**★★）',
        '> | ★これは取り出す |',
        '#### ★ 別の見出し',
        '> | ★これは取り出さない |',
      ]);
      expect(rows, hasLength(1));
      expect(rows.values.first, <String>['| ★これは取り出す |']);
    });

    test('★★ 実物に「写し」の節が在るなら★候補を持つ（★今日は 0 件である）★★', () {
      // ★★ 0 件そのものは★落とさない ★★
      //   ★**落とすと★★写しを持つ問いが 1 つも無い日に★永久に赤になる★★**（**D-25** の壊れ方）。
      //   ★**在るときだけ見る。★★無いことは★上の合成の群が受ける★★。**
    ...
    test('★★ 1 文字変えると★当たらない（★突き合わせが働いていること）★★', () {
      ...
      // ★★ 2026-09-04: ★実物の写しが 0 件になったので★合成の行を使う ★★
      //   ★**この字面は `docs/同期設計メモ.md` に実在する**（★★突き合わせの相手が要る★★）。
      const row = '| ★**限-2** | ★**一定の時間内に呼べる回数を決めて、超えたら断る** |';
      expect(

...
const _roots = <String>[
  ...
  '../loveca-db/lib',
  '../loveca-db/test',
  // ★★ パッケージを足したら★ここも足す（決定 D126-2 / 新所見 **D-31**）★★
  //   ★この `_roots` は**パッケージ名を名指ししている**ので、
  //   ★新しいパッケージは**既存の見張りに 1 つも入らない。**
  //   ★★走査の穴はそのまま完了条件の穴になる。★★
  '../loveca-server/lib',
  '../loveca-server/test',
];
...
///
/// ★★ 太字の飾りを跨ぐ ★★
/// この走査を最初に書いたときは `決定 D` の直後に数字を要求しており、
/// ★**`決定 **DNN**` の形を黙って飛ばしていた**（★実在した）。
/// → ★**狭めた走査はそのまま穴になる**（**D-31**）。★飾りだけを許して跨ぐ。
final _decisionRef = RegExp(r'決定 \*{0,2}(D\d+)');
...
void main() {
  group('★★ 陽性対照 —— 取り出しと突き合わせが働くこと ★★', () {
    ...
    test('★★ 太字の飾りが付いていても拾う（★最初の版が飛ばしていた形）★★', () {
      // ★★ 陽性対照 —— この 1 件が無いと、飾り付きは「無い」のか「見ていない」のか分からない ★★
      expect(decisionRefsIn('/// 決定 **D110** で決めた'), {'D110'});
      expect(decisionRefsIn('/// 決定 *D110* で決めた'), {'D110'});
      // ★対: 飾り以外を跨がない（★`決定 ` の直後に別の語が来たら参照ではない）。
      expect(decisionRefsIn('/// 決定 した D110'), isEmpty);
    });

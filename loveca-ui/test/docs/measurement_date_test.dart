...
/// ★★ 境目（★★1 行★★）★★
///
/// > ★★**当たった行を★★この commit で自分が★★触った★★なら（★書いた / ★消した）★同じ commit で直す。
/// >   ★既に出した commit の行なら★★別の commit で直す。**★★
///
/// ★★ 2026-09-04 訂正: ★★「書いた」では★実際に当てた場面を覆えなかった★★（**D-25** / **§7-8**）★★
///
/// ★**この境目を書いた★同じ回に、★★覆えない場面を 1 つ踏んだ★★** ——
/// ★**落ちたのは★★自分が★消した★行が原因である★★**（★`docs/利用者への問い.md` の Q-01 の節ごと閉じた
/// ／ ★★台帳が 1 → 0 の★減った側★★ / ★下の [_baseline] の注記）。
/// ★★**「書いた」にも「既に出した commit」にも入らない**★★ —— ★★どちらの枝にも当てはまらない行が在った★★。
/// → ★**字面を「★★触った（書いた / 消した）★★」に広げた。★★根拠は 1 ミリも動かない★★**
/// （★消したのも★その commit の論点の中で起きたことである）。
///
/// ★★**「数を合わせるな」も★減った側は覆っていない**★★ —— ★**下の [_baseline] の注記が書くとおり、
/// ★★減った側の直し方は『数を合わせる』である★★**（★★行そのものが文書から出たので「日付を書く」ができない★★）。
/// ★**その注記は 1 文字も書き換えない**（**D-35** —— ★★書いた時点で正しい★★）。
///
...
///
/// ★★**どちらの場合も★台帳（[_baseline]）の数は★★増やす向きには動かさない★★**★★ ——
/// ★**これは★★粒度とは別の規約である★★**（★上の「直し方は『日付を書く』であって『数を合わせる』ではない」）。
/// ★★**減る向きは別である**★★ —— ★**行が★文書から出たときだけ★下げる**（★2026-09-04 に 1 度起きた / ★[_baseline] の注記）。
///
/// ★★ 何を「実測」と見るか ★★

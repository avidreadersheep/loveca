...
///
/// ★★ 旧称は D88-1 で廃止した ★★
/// R7 は旧称で **1 人が両プレイヤーを操作する盤面**を出していたが、
/// 利用者の想定はそうではなく**対戦相手を置かず自分のデッキの動きだけを見るもの**だった。
/// **名前と実体が食い違っていた**ので、語ごと廃止した。
/// ★廃止した語そのものは `docs/決定事項一覧.md` の **D88-1** にだけ書いてある ——
/// コードとテストに残すと `grep` で新旧を区別できなくなるからで、
/// **語が戻っていないことは `test/board/abolished_term_test.dart` が機械で見ている。**
/// ★D88 以前の文書・コミットメッセージに出てくる旧称は**すべてローカル対戦**を指す。
///
/// | モード | 領域 | 秘匿 | 手番 |
/// |---|---|---|---|

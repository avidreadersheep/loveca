...
/// 探す（R4 と同じ一覧ペイン）→ 入れる（ドラッグ / 「+」）→ 検証を見る（検証パネル **P1**）が
...
/// | 論理削除（決定 D102） | ★検証パネル **P1** の常設 |
///
/// ★★ この段落の `P1` は UI のペイン番号（`docs/UI設計メモ.md` §2-3）である ★★
/// 同期の設計番号だった旧 `P1`〜`P5` とは別の体系で、後者は 2026-08-27 に
/// `決定 D100`〜`D102` / `D11` / `D35` へ置き換えて**コードから消えた**
/// (`ルール整合性チェック_v1.06.md` D-5)。★丸括弧で囲んだ `P1`〜`P5` は書かないこと ——
///   走査 `[（(]P[1-5][）)]` が旧体系の再発を見張れなくなる。
///

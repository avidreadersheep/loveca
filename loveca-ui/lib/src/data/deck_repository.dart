...
class DeckRepository {
  ...
  ///
  /// ★★ この層は [ops] を 1 度も読まない ★★
  /// ★**器だけを通す。**★中身を読んで行にするのは `DeckDao.save` である。
  ///
  /// ★★ ここには「★挙動は 1 つも変わらない」と書いてあった（2026-08-30 訂正）★★
  /// ★**書かれた時点（§17-9-7 の commit 4 / 署名だけ）では正しかった。**
  /// commit 5 で `DeckEditStore` が実際に列を渡すようになり、
  /// ★**渡した列の分だけログの行が増えるようになった**ので偽になった。
  /// ★型は `ルール整合性チェック_v1.06.md` **D-15 (l)**（★前提があとから動いた）。
  /// ★**走査では出ない** —— 字面は正しい日本語で、grep に引っかかる語が無い。
  Future<Deck> save(
    Deck base,
    DeckDraft draft, {
  ...
  /// ★★ 同期で受け取ったデッキを★★そのまま書く★★（★§32-6 の **24** / 決定 **D144**）★★
  ///
  /// ## ★★ なぜ [save] を使えないか（★★要求を満たさない★★）★★
  ///
  /// ★[save] は★★3 つ組を★この層で決める★★ —— ★`updatedAt` を [_clock] から、
  /// ★`revision` を `base.revision + 1` にする（★上の doc / **D101**）。
  /// → ★**受信に使うと★★送信側の `updatedAt` が★受信時刻に化ける★★。**
  /// → ★★**N-15（**D119-1**）が壊れる**★★ —— ★あれは「★★送信側の `updatedAt` をそのまま採る★★」である。
  /// → ★**そのうえ★★決着層（**D108-2**）が★`updatedAt` を見る★★ので、★★受信するたびに勝ち負けが変わる★★。**
  ///
  /// ★★**`docs/同期設計メモ.md` §4 が★`copyWith` について予告した害と★同じ形である**★★
  /// （★★あちらは `copyWith`、★こちらは `save`★★ —— ★**§4 は `save` を 1 文字も述べていない**）。
  ///
  /// ## ★★ **D115-4** の訂正ではない ★★
  ///
  /// ★**あれは「受け取りは★★既存の `save` 1 本で足りる★★」である。**
  /// ★★**`DeckDao.save` は★渡された `Deck` を★1 フィールドも変えずに書く**★★（★実読）。
  /// → ★**「1 本」は★★DAO の口である★★。★この層は★★方針を足しているだけ★★**（**D116-1** ＝ 3 つ組を決めるのは呼び出し側）。
  /// → ★**受信側の呼び出し側は「★★送信側の値をそのまま採る★★」を選んだ**（**D119-1**）。★**それがこの口である。**
  ///
  /// ## ★★ [ops] は★呼び出し側が決める ★★
  ///
  /// ★**解決が起きたなら `DeckEditOpKind.resolveConflict` を 1 件**（**D119-1**）。
  /// ★**相手側だけが変わっていたなら★★1 件も要らない★★**（★★手元の編集ではない★★）。
  /// → ★**この層は 1 度も読まない。★器だけを通す**（[save] と同じ）。
  Future<void> saveReceived(
    Deck received, {
    required List<DeckEditOpRecord> ops,
  }) =>
      guardRepository('deck.saveReceived', () async {
        await DeckDao(_db).save(received, ops: ops);
      });
  ...
  /// ★★ [Clock] の値そのもの（`docs/UI設計メモ.md` §9-1）★★
  /// 編集ログの `at` は**操作が起きた時刻**であって保存時刻ではないので、
  /// ★**9 操作のたびに 1 つ要る**（`DeckEditStore` が呼ぶ / 決定 **D110-1**）。
  ///
  /// ★**新しい時刻の口を作ったのではない。**`create` / `save` / `softDelete` が
  /// 使っているのと**同じ [Clock]** をそのまま返す。
  /// → ★`DateTime.now()` は `clock.dart` の 1 行のまま増えない（§9-1 の利得 2）。
  DateTime now() => _clock();

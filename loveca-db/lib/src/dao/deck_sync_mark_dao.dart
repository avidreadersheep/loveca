...
class DeckSyncMarkDao {
  ...
  /// ★★ 送り終えた編集ログを捨てる（★§32-6 の **28** / 決定 **D150-1** ＝ ★★捨-B★★）★★
  ///
  /// ## ★★ 「送り終えた」の定義は★目印である ★★
  ///
  /// ★**目印は「★★最後に送った操作の id★★」である**（**D140-1** ＝ 印-1）。
  /// → ★**捨てるのは★★その id 以下の行★★である。★★新しい量を 1 つも要らない★★。**
  /// ★**`schemaVersion` は上がらない**（★列も表も増えない / ★確かめた）。
  ///
  /// ## ★★ 行が無ければ★1 件も捨てない（決定 **D114-3**）★★
  ///
  /// ★**器の行の不在は「★★まだ一度も同期していない★★」である。**
  /// → ★★**送ったものが 1 つも無いので、★捨てるものも 1 つも無い。**★★
  /// ★**ここを飛ばすと★★未送信の編集を丸ごと消す★★**（★★この口で最も害の大きい取り違えである★★）。
  ///
  /// ## ★★ 目印より後ろは★1 行も触らない ★★
  ///
  /// ★**未送信の編集は★★残る★★**（★★捨-B が「失うものはない」と言えるのはこの一点である★★ / §77-2）。
  ///
  /// ## ★★ 器にも `decks` にも触れない ★★
  ///
  /// ★**消すのは `deck_edit_ops` の行だけである**（★目印は★★動かさない★★ —— ★次の同期が同じ答えを出す）。
  /// ★**冪等である** —— ★★2 度呼んでも★2 度目は 0 件である★★（★対で固定した）。
  ///
  /// ## ★★ 捨てるのは★このデッキの分だけである ★★
  ///
  /// ★**別のデッキのログは★1 行も触らない**（★目印はデッキごとに在る / **D125-9**）。
  ///
  /// ★捨てた件数を返す。
  Future<int> discardOpsUpToMark(String deckId) async {
    final row = await (db.select(db.deckSyncMarks)
          ..where((m) => m.deckId.equals(deckId)))
        .getSingleOrNull();
    // ★★ D114-3 —— ★行が無ければ「まだ一度も同期していない」★★
    if (row == null) return 0;
    return (db.delete(db.deckEditOps)
          ..where((o) =>
              o.deckId.equals(deckId) &
              o.id.isSmallerOrEqualValue(row.logMark)))
        .go();
  }

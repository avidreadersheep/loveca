...
class DeckDao {
  ...
  /// 検出層は「操作の履歴」である。★**意図を持っているのは上の層**（`DeckEditStore` の
  /// 9 つの名前つき操作）で、★**DAO は after だけを受け取る**
  /// （`docs/同期設計メモ.md` §15-7-2 の層の表）。→ ★**意図は引数で降ろすしかない。**
  ///
  /// ★★ なぜ引数で受け取るのか —— 入れ子を避けるためである ★★
  /// ログ行は**本体と同じトランザクション**で書かねばならない
  /// （★片方だけ残ると「行は変わったのにログが無い」/「ログは在るのに行が変わっていない」が作れる）。
  /// ★このメソッドは**既に自前のトランザクションを持っている**ので、
  /// ★**操作列をここまで引数で降ろせば、新しいトランザクションを 1 つも開かずに済む**（§17-9-3）。
  /// ★★**「入れ子がこの executor で成り立つ」と測ったのではない。★避けただけである**★★
  /// （★**§16-8 #5 は未測定のまま残る** / **D-28** の作法 / `softDelete` の doc と同じ断り）。
  ///
  /// ★★ 既定値を持たせない（`required` である）★★
  /// ★**既定値（`const []`）を置くと、新しい保存経路が [ops] を省いて呼べる。**
  /// ★コンパイルは通り、★**ログは黙って空のままになる。**
  /// → ★★**それは穴 (a) の形そのものである**★★ —— `DeckListStore.saveMeta` が
  /// `DeckEditStore` を通らず `DeckRepository.save` へ直行していた
  /// （`docs/同期設計メモ.md` §15-7-1 の 2）のを、★**既定値で作り直すことになる。**

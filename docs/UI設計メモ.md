...
| 段 | 手段 | 値 | 格 |
|---|---|---:|---|
| (1) | ウィジェットテスト（`test/ui/deck_pane_width_test.dart`） | 溢れない下限 **288** 論理px | ★**テスト用フォント**。「構造の下限」であって**読める幅ではない**（名前も刷り番号も ellipsis で潰れるだけ）。★**下の経緯を読むこと** |
...
★★**2026-08-27 の注意: (1) と (2) が同じ 288 になったのは偶然である。**★★
★**格は違う**（(1) はテスト用フォント / (2) は実機の実データ）。**同じ数だからと束ねないこと。**

★★**採用値 320 の余裕が 122 → 32 に減った。**★★
★**回帰ガードは通っている**（288 ≤ 320）ので **D61 の結論は変わらない**が、
★**(2) の実機の下限は測り直していない。**テスト用フォントは日本語の実幅ではないので、
**実機の下限は 288 より広い可能性がある。**
→ ★**次に実機を触る人へ**: 288 付近で「規則順に戻す」のラベルが折り返さないかを見ること。
★**U26**（袖の文言が語の途中で折り返す）と同じ型の問題が、ここでも起こりうる。
...
| ~~**U22**~~ | ~~★**デッキの並び順をメンバー = コスト降順 / ライブ = スコア降順にする。**~~ → ★**2026-08-27 確定（決定 D99）。**★**手動並べ替えは残して `ord` に保存する**。規則順は区分順 → メンバー = `cost` 降順 / ライブ = `score` 降順 / エネルギー = `printingId` 昇順、副次キーは `printingId` 昇順。★**null は末尾**。★★**D65 の backfill 基準は目的として維持し、値だけ差し替えた**★★（旧値のままだと既存デッキだけ永久に旧順で残る）。★**判断に使った材料は §12-2** | **解消**（`docs/決定事項一覧.md` D99） |

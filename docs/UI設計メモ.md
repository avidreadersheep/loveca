...
★カード詳細（R5）は種別で枠を選ぶ（`cardAspectRatioOf`）。
一覧（`CardGrid`）は D42 の測定条件のまま 200:279 で、**ライブのセルは `cover` で左右が切れる**
（§10 の **U11**）。
...
| **U11** | ★**一覧（`CardGrid`）はライブのセルを `cover` で切っている。**全セルが 200:279 固定で、ライブの絵は 200:143（§5-1）。比を変えると**セル幅 120 物理px という D42 の測定条件が動く**（`ResizeImage` の効果もキャッシュの見積りも測り直し）ので M5 では触っていない | **一覧の見た目を次に触るとき**（要求が出たら） |
| **U10** | ★**デッキの並び順が保存されない**（`deck_entries` に順序列が無い / §9-8）。方式は決定 **D65** に書いてある | **`loveca_db` を次に触るとき**（D-8 と同じ時点） |
| **U9** | ★**論理削除したデッキを戻す口が無い。**`deletedAt` を立てるだけなので DB には残る（P3）が、UI から復元できない。M2 は確認ダイアログ 1 枚で誤操作を防いでいるだけ | **M6**（R6 設定・診断を作るとき）**か、それ以降** |

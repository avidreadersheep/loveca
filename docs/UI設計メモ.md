...
★★**2026-09-03: ★覆した部分と★覆していない部分を分ける。★上の 12 行は 1 文字も書き換えない**★★（**D-35**）——
★**引き金は `docs/Android UI 決定.md` §1-5**（★★Android ではアプリが自分を再起動するのが行儀の悪い挙動になる★★ / ★利用者判断）。

| | |
|---|---|
| ★**覆した** | ★**取り込みが★★起動ゲート以外でも走る★★**（`BootController.reload`） |
| ★★**覆していない**★★ | ★★**上の「静かに古くなる」そのもの**★★ —— ★**1 ミリも消えていない**（★受け入れた穴 / ★申し送り §2 の穴 2） |

★★**手当ては「無効化を書く」ではなく「★★丸ごと作り直して差し替える★★」である**★★ ——
★**`AppEnvironment` を新しく組んで `BootReady` を出し直す**ので、★`AppScope` より下は新しいカタログを見る。
★★**ただし★既に開いている画面（`DeckEditStore` など）は★古いリポジトリを掴んだままである**★★
（★構築時に受け取っているため）。→ ★★**「無効化漏れを構造的に不可能にする」は★もう成り立たない**★★。

★★**呼ぶ側は 1 行も無い**★★（★2026-09-03 走査 —— ★★`reload()` を呼ぶ場所が `lib` に 1 つも無い★★）。
...
★★**2026-09-03: ★論点 1 の「3 つ目の道」の速さを測った**★★ ——
★**正は `docs/Android UI 決定.md` §15 である**（**D-15** の規約 3 —— ★★ここに写さない★★）。
★★**選んでいない。★論点 1 は閉じていない**★★（★比べる相手を 1 度も測っていない）。

...
| **U9** | ★**論理削除したデッキを戻す口が無い。**`deletedAt` を立てるだけなので DB には残る（決定 D102）が、UI から復元できない。M2 は確認ダイアログ 1 枚で誤操作を防いでいるだけ。★★**M6 では入れなかった。忘れたのではない**——`DeckDao.softDelete` が `revision` を上げない（**D-9**）のが未解決で、復元時に `revision` をどう扱うかはその判断が先に要る。決めないまま復元を作ると、Phase 4 の同期で**削除と復元の差分検出が両方壊れる** | ★**D-9 を決めたとき（Phase 4）** |

...
★★ 仕込みを★どうやって列挙するか（★2026-09-04 / 運転指示【0】(3)）★★

★★**引き金**★★ —— ★2026-09-03 の回は 71 通りを測り、★★「0 件」5 回のうち 4 回が (乙) だった★★。
★**密度は高かったが、★★どうやって列挙したかが★どこにも書かれていなかった★★。**
→ ★★**書かなければ、★次の回の密度は運になる。**★★

★★**下は★2026-09-03 の 5 つの表のラベルから★組み直したものと、★2026-09-04 の 12 通りの実際の手順である**★★
（★★spec そのものは残っていない★★ —— ★★推測が入っていることを★隠さない★★ / D-28）。

★★ 7 つの出どころ（★★上から順に当てる★★）★★

  ★**1) その回に「決めた」と書いたことを 1 つずつ反転する**
     ★出どころ ＝ ★★コミット本文と CLAUDE.md の記録の★断定の行★★
       （「〜しない」「〜する」「〜だけを見る」）。
     ★例: ★(A) 0 枚でも 0 を出す ／ ★(C) 「…」を出さない ／ ★(D) 段 3 を外す

  ★**2) 既定値を 1 つずつ動かす**
     ★出どころ ＝ ★★報告 §3 の「自分で決めた既定値」の全件★★（★★1 件 = 1 通り★★）。
     ★例: ★(N) 既定を降順にする ／ ★(D) 絞り込みを前方一致にする

  ★**3) 導いている値を★字面に置き換える（★逆向きも）**
     ★出どころ ＝ ★★「字面で書かない」と書いた箇所★★。
     ★例: ★(E) 4 枚制限を字面で書く ／ ★(F) 上限を字面で書く

  ★**4) 境界を 1 つずつ跨ぐ**
     ★出どころ ＝ ★★上限 / 下限 / 空 / null / 0 件★★。
     ★例: ★(B) 列の上限を 1 つ増やす ／ ★(K) 一覧が空のとき行ごと出さない

  ★**5) 走査・陽性対照そのものを 1 つずつ殺す**
     ★出どころ ＝ ★★その回に置いた走査★★（★D-30 / D-31 の受け）。
     ★例: ★(O) 走査からコメント外しを落とす ／ ★(E) 根から 1 つ外す ／ ★(C) 判定を常に真に

  ★**6) 2 か所に在るものを★片方だけ変える**
     ★出どころ ＝ ★★共有した所 / 同じ字面を両側が読む所★★（D-15 の規約 3 / D-37 の裏）。
     ★例: ★(M) 送る鍵の字面をアプリ側だけ変える ／ ★(K) Windows が自前で判定する

  ★**7) 呼び出しを落とす（★★構造の守り★★）**
     ★出どころ ＝ ★★振る舞いに 1 ビットも出ない守り★★（★共有していること / ★正が 1 か所であること）。
     ★★**これは値では見えない。★走査でしか見えない**★★（★下の「★守る対象を先に分ける」）。
     ★例: ★(N) フォームが列を使わず自分で回す ／ ★(I) 3 本のカウンタを自分で数える

★★ 完全性の当て方（★★1 行★★）★★

> ★★**`expect` の群を 1 つ書いたら、★その群だけを落とす仕込みが 1 通り在ること。**★★

★**無ければ★★その群は「0 件」になる**（＝ D-27 の (乙)）。★**列挙の時点で当てれば★測る前に分かる。**

★★ 守る対象を★先に分ける（★2026-09-04 / ★★これが (乙) の半分を生む★★）★★

| ★守る対象 | ★★仕込みは値に出るか★★ | ★対の形 |
|---|---|---|
| ★★**振る舞い**★★（★出る値 / ★画面の字面） | ★★**出る**★★ | ★値を見る |
| ★★**構造**★★（★正が 1 か所であること / ★共有していること） | ★★**出ない**★★ | ★★**走査で見るしかない**★★ |

★★**構造の守りに★振る舞いの対を置くと★必ず 0 件になる**★★ ——
★**「呼びつつ、★隣で自分でも回す」形は★★出る値が 1 つも変わらない★★。**
→ ★**先に分ければ★★測る前に★走査を置ける★★。**

★★ 覆わないもの（★言い切る）★★
★**1) 上の 7 つで★全部かは★測っていない**（★標本は 2 回分である / D-28）。
★**2) 「1 群 = 1 通り」は★★下限であって上限ではない★★**（★1 つの群に 2 通り当たることは在る）。
★**3) 機械にできない** —— ★★出どころは★その回に何を決めたかであり、★リポジトリに 1 バイトも残らない★★。
